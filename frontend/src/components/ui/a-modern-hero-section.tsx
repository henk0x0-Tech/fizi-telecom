import { Bot, Cpu, BrainCircuit, Sparkles, Globe } from 'lucide-react';

const companies = [
  { name: "OpenAI", icon: BrainCircuit },
  { name: "Google AI", icon: Globe },
  { name: "Microsoft", icon: Cpu },
  { name: "Meta AI", icon: Sparkles },
            ...
            {companies.map((company, index) => {
              ...
              return (
                <div key={index} className="flex items-center space-x-2">
                  <Icon className="w-4 h-4 sm:w-6 sm:h-6 text-gray-500" />
                  <span className="text-gray-600 font-medium text-sm sm:text-base">{company.name}</span>
                </div>
              );
            })}
          </div>
